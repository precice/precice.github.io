...
* `writeVectorGradientData` writes gradient data corresponding to vector-valued data, i.e, a Jacobian matrix. The matrix is passed as a 1D-array. The parameter `columnsFirst` allows to select whether the 1D-array contains the data column-wise or row-wise.
...
When using the `columnsFirst` parameter, the following format is required:
