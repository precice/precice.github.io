...
When using faces mesh, instead of a node set (`*NSET`), a `*SURFACE` must be sent, defined by a list of elements and face numbers. Instead of starting with a `N`, the name must start with a `S`. For example: `SCalculiX_Mesh` (while in the preCICE configuration the name remains `CalculiX_Mesh`).
