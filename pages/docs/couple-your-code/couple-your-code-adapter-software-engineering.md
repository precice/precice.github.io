...
What we develop in the [step-by-step guide](couple-your-code-preparing-your-solver) is best described as an _adapted code_, not an _adapter_. We directly modified the main solver routines. There are also ways to separate the adapter code from the solver code, making the adapter easier to maintain and distribute/publish:
...
{% tip %}
In any case, fully integrating the adapter into the codebase and the continuous integration system of the solver (with any of the methods described here) would make maintenance easier, as most complications typically come from changes in the solver code, not from preCICE (in which case, you can refer to a [porting guide](./couple-your-code-porting-overview.html)).
{% endtip %}
